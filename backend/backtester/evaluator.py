...
class StrategyEvaluator:
    ...
    def evaluate(self, data: pd.DataFrame) -> EvaluationResult:
        ...
        # Extract factor data if present
        # Look for columns that might be factors (not in basic OHLCV columns)
        factor_columns = [col for col in data.columns if col not in required_columns]
        factor_data = None
        
        if factor_columns:
            # Check if this strategy requires factors
            required_factors = self.strategy.get_required_factors() if hasattr(self.strategy, 'get_required_factors') else []
            
            if required_factors:
                # Create a DataFrame with just the required factors
                factor_data = pd.DataFrame(index=data.index)
                for factor in required_factors:
                    if factor in data.columns:
                        factor_data[factor] = data[factor]
                    else:
                        # If a required factor is missing, try to calculate it
                        if factor == 'ma' and 'sma_20' in data.columns:
                            # Use SMA 20 as a substitute for general MA
                            factor_data['ma'] = data['sma_20']
                        elif factor == 'std':
                            # Calculate standard deviation on the fly if needed
                            std = data.groupby(level='ticker')['close'].transform(
                                lambda x: x.rolling(window=20, min_periods=1).std()
                            )
                            factor_data['std'] = std
                        else:
                            raise ValueError(f"Required factor {factor} not available for {self.strategy.__class__.__name__}")
        
        # Get strategy signals
        if factor_data is not None:
            signals = self.strategy.generate_signals(data, factor_data)
        else:
    ...
    def _calculate_positions(self, signals: pd.DataFrame, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Check if signals are enum values (like SignalType)
        signal_values = signals['signal']
        
        # Convert enum values to integers if necessary
        if hasattr(signal_values.iloc[0], 'value'):
            # It's an enum with a value attribute
            positions['position'] = signal_values.apply(lambda s: s.value)
        elif isinstance(signal_values.iloc[0], str):
            # It's a string representation
            signal_map = {'BUY': 1, 'SELL': -1, 'HOLD': 0}
            positions['position'] = signal_values.map(signal_map).fillna(0).astype(int)
        else:
            # Try regular conversion
            positions['position'] = signal_values.astype(int)
        ...
        for ticker in data.index.get_level_values('ticker').unique():
            ...
            for i in range(len(change_points) - 1):
                try:
                    ...
                    # Convert MultiIndex dates to string
                    entry_date_str = entry_date[0].isoformat() if isinstance(entry_date, tuple) else entry_date.isoformat()
                    exit_date_str = exit_date[0].isoformat() if isinstance(exit_date, tuple) else exit_date.isoformat()
                    ...
                    # If anything goes wrong extracting a trade, log and continue
                    print(f"Error extracting trade: {str(e)}")
                    continue
