...
class PerformanceMetrics:
    ...
    def calculate(self, equity_curve: pd.DataFrame, drawdown_curve: pd.DataFrame, trades: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        if len(returns) > 1 and total_return != 0:
            try:
                # Add small epsilon to avoid negative values in power calculation
                annualized_return = ((1 + max(total_return, 0)/100) ** (252/len(returns)) - 1) * 100
            except (ValueError, RuntimeWarning):
                # Fallback to simplified calculation if power calculation fails
