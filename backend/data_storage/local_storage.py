...
class LocalStorage(DataStorageBase):
    ...
    def save_data(self, 
        ...
        """
        try:
            # Validate data based on dataset type
            # Only apply strict validation for market_data
            if dataset_name == "market_data" or dataset_name == "raw_market_data" or dataset_name == "processed_data":
                self.validate_data(data, dataset_type="market_data")
            else:
                # For other dataset types, just check if it's a valid DataFrame
