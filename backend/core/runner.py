...
class TradingRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            storage_manager: Optional storage manager for data persistence
        """
        self.config = config
    ...
    def run_backtest(self) -> Dict[str, Any]:
        ...
        # Store raw historical data if storage manager is available
        if self.storage_manager:
            try:
                version_id = self.storage_manager.store_generic_data(
                    data, 
                    "raw_market_data",
                    metadata={
                        "source": "backtest_raw",
                ...
                self.logger.info(f"Stored raw market data with version_id={version_id}")
            except Exception as e:
                self.logger.error(f"Failed to store raw market data: {str(e)}")
        
        # Process data with factors
        processed_data = self._process_data(data)
        
        # Calculate all needed factors for the strategies
        factor_data = self.calculate_factors(processed_data)
        
        # Add factor columns to processed data for the evaluator
        for factor_name, factor_series in factor_data.items():
            processed_data[factor_name] = factor_series
        
        # Store processed data if storage manager is available
        if self.storage_manager:
            try:
                version_id = self.storage_manager.store_generic_data(
                    processed_data, 
                    "processed_data",
                    metadata={
                        "source": "backtest_processed",
                        "run_id": run_id,
                        "timestamp": datetime.now().isoformat(),
                        "applied_factors": [f.__class__.__name__ for f in self.factors]
                    }
                )
                self.logger.info(f"Stored processed data with version_id={version_id}")
            except Exception as e:
        ...
        for strategy in self.strategies:
            ...
            # Initialize evaluator for this strategy
            ...
            # Evaluate strategy
            results = evaluator.evaluate(processed_data)
            
            # Add commission and slippage from config
            results.metrics['commission'] = self.config["portfolio"]["commission"]
            results.metrics['slippage'] = self.config["portfolio"]["slippage"]
            
            strategy_results[strategy.__class__.__name__] = results
            
            # Generate report
            self._generate_backtest_report(results, strategy)
            
            # Store strategy results if storage manager is available
            if self.storage_manager:
                try:
                    version_id = self.storage_manager.store_backtest_results(
                        results,
                    ...
                    self.logger.info(f"Stored backtest results for {strategy.__class__.__name__} with version_id={version_id}")
                except Exception as e:
                    self.logger.error(f"Failed to store backtest results for {strategy.__class__.__name__}: {str(e)}")
        
        # Return combined results
        # Since the API has changed, we can't use evaluator.get_results() anymore
    ...
    def _trading_loop(self) -> None:
        ...
        5. Updates portfolio
        6. Manages risk
        7. Stores data for persistence and analysis
        """
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Store the raw market data if we have a storage manager
                if self.storage_manager:
                    try:
                        version_id = self.storage_manager.store_generic_data(
                            data, 
                            ...
                            metadata={
                                ...
                                "timestamp": datetime.now().isoformat(),
                                "config": {
                                    "interval": self.config["data"].get("interval", "1m"),
                                    "symbols": self._get_universe_symbols()
                                }
                            }
                        )
                ...
                # Store the processed data if we have a storage manager
                if self.storage_manager:
                    try:
                        version_id = self.storage_manager.store_generic_data(
                            processed_data, 
                ...
                # Store the signals if we have a storage manager
                if self.storage_manager and not signals.empty:
                    try:
                        version_id = self.storage_manager.store_generic_data(
                            signals, 
                            "signal_data",
                        ...
                        self.logger.info(f"Stored signals with version_id={version_id}")
                    except Exception as e:
                        self.logger.error(f"Failed to store signals: {str(e)}")
                ...
                # Store portfolio state if we have a storage manager
                if self.storage_manager:
                    try:
                        # Convert portfolio to DataFrame for storage
                        ...
                        self.logger.info(f"Stored portfolio snapshot with version_id={version_id}")
                    except Exception as e:
                        self.logger.error(f"Failed to store portfolio snapshot: {str(e)}")
    ...
    def _generate_backtest_report(
        ...
        Args:
            results: Backtest results as an EvaluationResult object
